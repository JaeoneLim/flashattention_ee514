...
struct SharedStorageQKVO {
    ...
    struct {
        cutlass::arch::ClusterBarrier barrier_Q;
        cutlass::arch::ClusterBarrier barrier_O;
        typename cutlass::PipelineAsync<kStages>::SharedStorage pipeline_k;
        typename cutlass::PipelineAsync<kStages>::SharedStorage pipeline_v;
...
struct Flash_fwd_kernel_traits {
    ...
    // Replace GMMA with WMMA for A100 compatibility
    using TiledMma0 = cutlass::gemm::warp::MmaTensorOp<
        cutlass::gemm::GemmShape<16, 16, 16>, // Warp tile shape
        cutlass::gemm::GemmShape<16, 16, 16>, // MMA operation tile shape
        Element,                              // A matrix data type
        cutlass::layout::RowMajor,            // A matrix layout
        Element,                              // B matrix data type
        cutlass::layout::ColumnMajor,         // B matrix layout
        ElementAccum,                         // Accumulator data type
        cutlass::layout::RowMajor             // C matrix layout
    >;

    using TiledMma1 = cutlass::gemm::warp::MmaTensorOp<
    ...
    using SmemLayoutAtomQ = cutlass::layout::RowMajor;
    using SmemLayoutQ = cutlass::layout::RowMajor;
    ...
    using SmemLayoutAtomK = cutlass::layout::ColumnMajor;
    using SmemLayoutK = cutlass::layout::ColumnMajor;

    using SmemLayoutAtomV = cutlass::layout::RowMajor;
    using SmemLayoutV = cutlass::layout::RowMajor;

    // Note this is the transpose in terms of the view, not in terms of memory.
    using SmemLayoutVt = cutlass::layout::RowMajor;

    using SmemLayoutAtomO = cutlass::layout::RowMajor;
    using SmemLayoutO = cutlass::layout::RowMajor;
...
struct SharedStorageQKVdOdKV<true, kStages, Element, OutputType, SmemLayoutQ, SmemLayoutdO,
    ...
    struct {
        ...
        cutlass::arch::ClusterBarrier barrier_K;
        cutlass::arch::ClusterBarrier barrier_V;
        typename cutlass::PipelineAsync<kStages>::SharedStorage pipeline_q;
        typename cutlass::PipelineAsync<kStages>::SharedStorage pipeline_do;
    };
};
...
struct SharedStorageQKVdOdKVSeqqPar<true, kStages, Element, OutputType, SmemLayoutQ, SmemLayoutdO,
    ...
    struct {
        cute::uint64_t tma_load_mbar[8]; // 8 TMA barrier pre-allcoated for usage.
        cutlass::arch::ClusterBarrier barrier_Q;
        cutlass::arch::ClusterBarrier barrier_dO;
        typename cutlass::PipelineAsync<kStages>::SharedStorage pipeline_k;
        typename cutlass::PipelineAsync<kStages>::SharedStorage pipeline_v;
    };
};
...
//     // SM80 아키텍처에 맞춘 GmemTiledCopyQdO 정의
//     using GmemTiledCopyQdO = decltype(
//         make_tiled_copy(
//             Copy_Atom<cute::SM80_CP_ASYNC, Element>{}, // SM80에서의 비동기 복사
//             Layout<Shape<Int<kBlockM>, Int<kHeadDim>>, Stride<_1, Int<kBlockM>>>{},
//             Layout<Shape<_1, _8>>{} // 8 elements per thread
//         )
//     );

//     // SM80 아키텍처에 맞춘 GmemTiledCopyKV 정의
...
//     // SM80 아키텍처에 맞춘 GmemTiledCopydKV 정의
...
//     // PdS (Probability Distribution S)
//     using SmemCopyAtomPdS = Copy_Atom<
//         std::conditional_t<!SdP_swapAB, cute::SM80_U32x4_LDSM_N, cute::SM80_U16x8_LDSM_T>,
//         Element>;

//     // dKV (Derivative of Keys and Values)
//     using SmemCopyAtomdKV = Copy_Atom<
//         std::conditional_t<!dKV_swapAB, cute::SM80_U32x4_LDSM_N, cute::SM80_U16x8_LDSM_T>,
//         Element>;

//     // dQ (Derivative of Queries)
//     using SmemCopyAtomdQ = Copy_Atom<
//         std::conditional_t<!dQ_swapAB, cute::SM80_U32x4_LDSM_N, cute::SM80_U16x8_LDSM_T>,
//         Element>;
...
//     // using MainloopPipeline = typename cutlass::PipelineAsync<kStages * 2>;
...
//     // Replace GMMA with WMMA for SM80 compatibility
//     using TiledMmaSdP = cutlass::gemm::warp::MmaTensorOp<
//         cutlass::gemm::GemmShape<16, 16, 16>, // Warp tile shape
//         cutlass::gemm::GemmShape<16, 16, 16>, // MMA operation tile shape
//         Element,                              // A matrix data type
//         cutlass::layout::RowMajor,            // A matrix layout
//         Element,                              // B matrix data type
//         cutlass::layout::RowMajor,            // B matrix layout
//         ElementAccum,                         // Accumulator data type
//         cutlass::layout::RowMajor             // C matrix layout
//     >;
...
//     // Replace GMMA with WMMA
//     using TiledMmadKV = cutlass::gemm::warp::MmaTensorOp<
//         cutlass::gemm::GemmShape<16, 16, 16>, // Warp tile shape
//         cutlass::gemm::GemmShape<16, 16, 16>, // MMA operation tile shape
//         Element,                              // A matrix data type
//         cutlass::layout::RowMajor,            // A matrix layout
...
//     static constexpr cutlass::gemm::warp::MmaTensorOp::Major MmadQMajorA = cutlass::layout::RowMajor;
//     static constexpr cutlass::gemm::warp::MmaTensorOp::Major MmadQMajorB = cutlass::layout::ColumnMajor;

//     // Replace GMMA with WMMA
//     using TiledMmadQ = cutlass::gemm::warp::MmaTensorOp<
//         cutlass::gemm::GemmShape<16, 16, 16>, // Warp tile shape
//         cutlass::gemm::GemmShape<16, 16, 16>, // MMA operation tile shape
//         Element,                              // A matrix data type
//         cutlass::layout::RowMajor,            // A matrix layout
...
//     // Shared memory layouts remain unchanged as they are compatible with WMMA.
//     using SharedStorage = std::conditional_t<
//         !Is_WS,
//         SharedStorageQKVdOdKV<!SdP_swapAB, kStages, Element, Element, SmemLayoutQ, SmemLayoutdO,
//                               SmemLayoutK, SmemLayoutV, SmemLayoutP, SmemLayoutdS, SmemLayoutdK, SmemLayoutdV>,
...
//     // using GmemTiledCopyKV = cute::SM90_TMA_LOAD;
//     // using GmemTiledCopydKV = cute::SM90_TMA_STORE;
//     // QdO (Queries and Outputs)
//     using GmemTiledCopyQdO = decltype(
//         make_tiled_copy(
//             Copy_Atom<cute::SM80_U32x4_CP_ASYNC, Element>{},
//             ClusterShape_MNK{},
//             Layout<Shape<_1, _8>>{})); // Adjust layout as needed

//     // KV (Keys and Values)
//     using GmemTiledCopyKV = decltype(
//         make_tiled_copy(
//             Copy_Atom<cute::SM80_U32x4_LDSM_N, Element>{},
//             ClusterShape_MNK{},
//             Layout<Shape<_1, _8>>{})); // Adjust layout as needed
...
//     // using SmemCopyAtomQ = Copy_Atom<cute::SM75_U32x4_LDSM_N, Element>;
//     // PdS (Probabilities and Derivatives of Softmax)
//     using SmemCopyAtomPdS = Copy_Atom<
//         std::conditional_t<!SdP_swapAB, cute::SM80_U32x4_LDSM_N, cute::SM80_U16x8_LDSM_T>,
//         Element>;
...
struct Flash_bwd_seqqpar_kernel_traits {
    ...
    // Shared memory layouts remain unchanged as they are compatible with WMMA.
    using SharedStorage = SharedStorageQKVdOdKVSeqqPar<!SdP_swapAB, kStages, Element, Element, SmemLayoutQ, SmemLayoutdO,
        SmemLayoutK, SmemLayoutV, SmemLayoutP, SmemLayoutdQTMA>;
